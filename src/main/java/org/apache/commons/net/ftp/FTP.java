 ...
 * have
 * to use the FTP class. The {@link org.apache.commons.net.ftp.FTPClient} class,
 ...
 * to
 * interact with the FTP protocol and implement their own clients. A set of
 ...
 * {@code FTPConectionClosedException} is a
 ...
 * properly clean up the system resources used by FTP. Before disconnecting, you
 * may
...
public class FTP extends SocketClient {
    ...
    /**
     * A constant used to indicate the file(s) being transferred should be treated
     * as EBCDIC. Note however that there are several EBCDIC formats. All
    ...
    /**
     * A constant used to indicate the file(s) being transferred should be treated
     * as a local type. All constants ending in {@code FILE_TYPE} are used to
     * indicate file types.
     */
     ...
     * the default format. All constants ending in {@code TEXT_FORMAT} are used
     ...
     * control characters. All constants ending in {@code TEXT_FORMAT} are used to
     ...
     * characters. All constants ending in {@code TEXT_FORMAT} are used to
     * indicate text formatting for text transfers (both ASCII and EBCDIC).
     */
     ...
     * All constants ending in {@code _STRUCTURE} are used to indicate file
     ...
     * indexed pages. All constants ending in {@code _STRUCTURE} are used to
     ...
     * blocks. All constants ending in {@code TRANSFER_MODE} are used to indicate
     * file transfer modes.
     */
     ...
     * data. All constants ending in {@code TRANSFER_MODE} are used to indicate
     ...
     * (un)compressing data in the "deflate" compression format. All constants
     * ending in
     * {@code TRANSFER_MODE} are used to indicate file transfer modes.
     *
     * See the Internet Draft <a href=
     * "https://datatracker.ietf.org/doc/html/draft-preston-ftpext-deflate-04">Deflate
     ...
     * connection. The default encoding is an ASCII-compatible encoding. Some FTP
     * servers expect other encodings. You can change the encoding used by an FTP
     * instance with {@link #setControlEncoding setControlEncoding}.
     */
    ...
    /**
     * This is used to signal whether a block of multiline responses beginning with
     * xxx must be terminated by the same numeric code xxx See section 4.2 of RFC
     * 959 for details.
     */
    protected boolean strictMultilineParsing;

    /**
     * If this is true, then non-multiline replies must have the format: 3-digit
     * code <space> <text> If false, then the 3-digit code does not have to be
     ...
     * SocketClient._input_. This member starts with a null value, is initialized in
     * {@link #_connectAction_}, and set to null in {@link #disconnect}.
     */
     ...
     * control connection. Do not access the control connection via
     * SocketClient._output_. This member starts with a null value, is initialized
     * in {@link #_connectAction_}, and set to null in {@link #disconnect}.
     */
    protected BufferedWriter _controlOutput_;

    /**
     * The default FTP constructor. Sets the default port to {@code DEFAULT_PORT}
     * and initializes internal data structures for saving FTP reply
     * information.
     */
    public FTP() {
    ...
    /**
     * Gets the reply, but don't pass it to command listeners. Used for keep-alive
     * processing only.
     *
     * @since 3.0
    ...
    /**
     * A convenience method to send the FTP ABOR command to the server, receive the
     ...
     *                                      sending the command or receiving the
     *                                      server reply.
     */
    ...
    /**
     * A convenience method to send the FTP ACCT command to the server, receive the
     * reply, and return the reply code.
     *
    ...
    /**
     * A convenience method to send the FTP ALLO command to the server, receive the
     * reply, and return the reply code.
     *
    ...
    /**
     * A convenience method to send the FTP APPE command to the server, receive the
     * reply, and return the reply code. Remember, it is up to you to manage the
     * data connection. If you don't need this low level of access, use
     * {@link org.apache.commons.net.ftp.FTPClient} , which will handle all low
     * level details
     * for you.
     *
     * @param pathname The pathname to use for the file when stored at the remote
     *                 end of the transfer.
     * @return The reply code received from the server.
     * @throws FTPConnectionClosedException If the FTP server prematurely closes the
     *                                      connection as a result of the client
    ...
    /**
     * A convenience method to send the FTP CDUP command to the server, receive the
     * reply, and return the reply code.
     *
     * @return The reply code received from the server.
     * @throws FTPConnectionClosedException If the FTP server prematurely closes the
    ...
    /**
     * A convenience method to send the FTP CWD command to the server, receive the
     * reply, and return the reply code.
     *
    ...
    /**
     * A convenience method to send the FTP DELE command to the server, receive the
     * reply, and return the reply code.
     *
     ...
     * internal data so that the memory may be reclaimed by the garbage collector.
     * The
     * reply text and code information from the last command is voided so that the
     * memory it used may be reclaimed. Also sets {@link #_controlInput_} and
    ...
    /**
     * A convenience method to send the FTP EPRT command to the server, receive the
     * reply, and return the reply code.
     *
     * Examples:
     * <ul>
    ...
    /**
     * A convenience method to send the FTP EPSV command to the server, receive the
     * reply, and return the reply code. Remember, it's up to you to interpret the
    ...
    /**
     * Fetches a reply from the FTP server and returns the integer reply code. After
     * calling this method, the actual reply text can be accessed from either
     * calling {@link #getReplyString getReplyString } or {@link #getReplyStrings
     * getReplyStrings }. Only use this method if you are implementing your own FTP
    ...
    /**
     * Returns the nth line of text from the last FTP server response as a string.
     * The end of line markers of each are stripped from the line.
     *
    ...
    /**
     * Returns the lines of text from the last FTP server response as an array of
     * strings, one entry per line. The end of line markers of each are stripped
     * from
     * each line.
     *
     * @return The lines of text from the last FTP response as an array.
    ...
    /**
     * A convenience method to send the FTP HELP command to the server, receive the
     * reply, and return the reply code.
     *
     * @return The reply code received from the server.
     * @throws FTPConnectionClosedException If the FTP server prematurely closes the
    ...
    /**
     * Return whether strict multiline parsing is enabled, as per RFC 959, section
     * 4.2.
     *
     ...
     * section 4.2.
     * <p>
     * The default is true, which requires the 3-digit code be followed by space and
     * some text. <br>
     * If false, only the 3-digit code is required (as was the case for versions up
     * to 3.5) <br>
     *
    ...
    /**
     * A convenience method to send the FTP LIST command to the server, receive the
     * reply, and return the reply code. Remember, it is up to you to manage the
     * data connection. If you don't need this low level of access, use
     * {@link org.apache.commons.net.ftp.FTPClient} , which will handle all low
     * level details
     ...
     * for you.
     *
     * @param pathname The pathname to list, may be {@code null} in which case the
    ...
    /**
     * A convenience method to send the FTP MFMT command to the server, receive the
     * reply, and return the reply code.
     *
    ...
    /**
     * A convenience method to send the FTP MKD command to the server, receive the
     * reply, and return the reply code.
     *
    ...
    /**
     * A convenience method to send the FTP MLSD command to the server, receive the
     * reply, and return the reply code. Remember, it is up to you to manage the
     * data connection. If you don't need this low level of access, use
     * {@link org.apache.commons.net.ftp.FTPClient} , which will handle all low
     * level details
     ...
     * @return The reply code received from the server, may be {@code null} in which
     *         case the command is sent with no parameters
     * @throws FTPConnectionClosedException If the FTP server prematurely closes the
     *                                      connection as a result of the client
     *                                      being idle or some other reason
     *                                      causing the server to send FTP reply
    ...
    /**
     * A convenience method to send the FTP MLST command to the server, receive the
     * reply, and return the reply code. Remember, it is up to you to manage the
     * data connection. If you don't need this low level of access, use
     * {@link org.apache.commons.net.ftp.FTPClient} , which will handle all low
     * level details
    ...
    /**
     * A convenience method to send the FTP MODE command to the server, receive the
     * reply, and return the reply code.
     *
     * @param mode The transfer mode to use (one of the {@code TRANSFER_MODE}
     *             constants).
     * @return The reply code received from the server.
     * @throws FTPConnectionClosedException If the FTP server prematurely closes the
     *                                      connection as a result of the client
    ...
    /**
     * A convenience method to send the FTP NLST command to the server, receive the
     * reply, and return the reply code. Remember, it is up to you to manage the
     * data connection. If you don't need this low level of access, use
     * {@link org.apache.commons.net.ftp.FTPClient} , which will handle all low
     * level details
    ...
    /**
     * A convenience method to send the FTP NOOP command to the server, receive the
     * reply, and return the reply code.
     *
     * @return The reply code received from the server.
     * @throws FTPConnectionClosedException If the FTP server prematurely closes the
    ...
    /**
     * A convenience method to send the FTP PASS command to the server, receive the
     * reply, and return the reply code.
     *
    ...
    /**
     * A convenience method to send the FTP PASV command to the server, receive the
     * reply, and return the reply code. Remember, it's up to you to interpret the
     * reply string containing the host/port information.
     *
     * @return The reply code received from the server.
    ...
    /**
     * A convenience method to send the FTP PORT command to the server, receive the
     * reply, and return the reply code.
     *
     * @param host The host owning the port.
     * @param port The new port.
    ...
    /**
     * A convenience method to send the FTP PWD command to the server, receive the
     * reply, and return the reply code.
     *
     * @return The reply code received from the server.
     * @throws FTPConnectionClosedException If the FTP server prematurely closes the
    ...
    /**
     * A convenience method to send the FTP REIN command to the server, receive the
     * reply, and return the reply code.
     *
     * @return The reply code received from the server.
     * @throws FTPConnectionClosedException If the FTP server prematurely closes the
    ...
    /**
     * A convenience method to send the FTP REST command to the server, receive the
     * reply, and return the reply code.
     *
    ...
    /**
     * A convenience method to send the FTP RETR command to the server, receive the
     * reply, and return the reply code. Remember, it is up to you to manage the
     * data connection. If you don't need this low level of access, use
     * {@link org.apache.commons.net.ftp.FTPClient} , which will handle all low
     * level details
    ...
    /**
     * A convenience method to send the FTP RMD command to the server, receive the
     * reply, and return the reply code.
     *
    ...
    /**
     * A convenience method to send the FTP RNFR command to the server, receive the
     * reply, and return the reply code.
     *
    ...
    /**
     * A convenience method to send the FTP RNTO command to the server, receive the
     * reply, and return the reply code.
     *
     ...
     * numerical response code. After invocation, for more detailed information, the
     * actual reply text can be accessed by calling {@link #getReplyString
    ...
    /**
     * Sends an FTP command with no arguments to the server, waits for a reply and
     * returns the numerical response code. After invocation, for more detailed
     * information, the actual reply text can be accessed by calling
     * {@link #getReplyString getReplyString } or {@link #getReplyStrings
     * getReplyStrings }.
     *
     * @param command The FTPCommand constant corresponding to the FTP command to
     *                send.
     * @return The integer value of the FTP reply code returned by the server in
     *         response to the command.
     * @throws FTPConnectionClosedException If the FTP server prematurely closes the
    ...
    /**
     * Saves the character encoding to be used by the FTP control connection. Some
     * FTP servers require that commands be issued in a non-ASCII encoding like
     * UTF-8 so that file names with multi-byte character representations (e.g, Big
     * 8) can be specified.
     * <p>
    ...
    /**
     * A convenience method to send the FTP SITE command to the server, receive the
     * reply, and return the reply code.
     *
    ...
    /**
     * A convenience method to send the FTP SIZE command to the server, receive the
     * reply, and return the reply code.
     *
     * @param parameters The site parameters to send.
     * @return The reply code received from the server.
    ...
    /**
     * A convenience method to send the FTP SMNT command to the server, receive the
     * reply, and return the reply code.
     *
    ...
    /**
     * A convenience method to send the FTP STAT command to the server, receive the
     * reply, and return the reply code.
     *
     * @return The reply code received from the server.
     * @throws FTPConnectionClosedException If the FTP server prematurely closes the
    ...
    /**
     * A convenience method to send the FTP STOR command to the server, receive the
     * reply, and return the reply code. Remember, it is up to you to manage the
     * data connection. If you don't need this low level of access, use
     * {@link org.apache.commons.net.ftp.FTPClient} , which will handle all low
     * level details
    ...
    /**
     * A convenience method to send the FTP STOU command to the server, receive the
     * reply, and return the reply code. Remember, it is up to you to manage the
     * data connection. If you don't need this low level of access, use
     * {@link org.apache.commons.net.ftp.FTPClient} , which will handle all low
     * level details
    ...
    /**
     * A convenience method to send the FTP STRU command to the server, receive the
     * reply, and return the reply code.
     *
     * @param structure The structure of the file (one of the {@code _STRUCTURE}
     *                  constants).
     * @return The reply code received from the server.
     * @throws FTPConnectionClosedException If the FTP server prematurely closes the
     *                                      connection as a result of the client
    ...
    /**
     * A convenience method to send the FTP SYST command to the server, receive the
     * reply, and return the reply code.
     *
     * @return The reply code received from the server.
     * @throws FTPConnectionClosedException If the FTP server prematurely closes the
    ...
    /**
     * A convenience method to send the FTP TYPE command to the server, receive the
     * reply, and return the reply code.
     *
    ...
    /**
     * A convenience method to send the FTP USER command to the server, receive the
     * reply, and return the reply code.
     *
